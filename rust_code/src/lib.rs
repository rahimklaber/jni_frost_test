...
use modular_frost::dkg::{
    frost::{Commitments, KeyGenMachine, KeyMachine, SecretShare, SecretShareMachine},
    ThresholdKeys, ThresholdParams,
};
use modular_frost::{ThresholdCore, Participant};
use modular_frost::{
    algorithm::Schnorr,
    curve::{Ciphersuite, Secp256k1},
    sign::{
        AlgorithmMachine, AlgorithmSignMachine, AlgorithmSignatureMachine, Preprocess,
        PreprocessMachine, SignMachine, SignatureMachine, Writable,
    },
};
use rand::rngs::OsRng;
...
impl SchnorrSingleSignTest {
    ...
    fn new() -> Self {
        // KeyPair::from
        Self::new_from_bytes(
            hex::decode("3e6c3783aa5cf8952b0d7f4e7942c6556b262b3d903679747f93ebd9a5730105")
                .unwrap()
                .as_slice(),
        )
        // return Self{
    ...
    fn sign_tx(&self, msg_i8: &[i8], prev_out_script: &[i8]) -> Vec<i8> {
        ...
        let script = Script::new_v1_p2tr_tweaked(TweakedPublicKey::dangerous_assume_tweaked(
            tweaked
                .clone()
                .to_inner()
                .public_key()
        ...
        println!(
            "tweaked priv: {:?}",
            hex::encode(tweaked.clone().to_inner().secret_bytes())
        );
        ...
        println!(
            "bitcoin rust serialized: {:?}",
            hex::encode(tx_to_sign.clone().serialize())
        );

        sig.verify(&msg, &tweaked.clone().to_inner().x_only_public_key().0)
            .unwrap();
...
impl SchnorrKeyWrapper {
    ...
    fn from_serialized(key_share: &[i8]) -> Self {
        ...
        Self {
            key: ThresholdKeys::new(ThresholdCore::read::<&[u8]>(&mut key_share.as_ref()).unwrap()),
        }
    }
    ...
    fn get_bitcoin_encoded_key(&self) -> Vec<i8> {
        ...
        let mut pubkey_obj =
            secp256k1::XOnlyPublicKey::from_slice(&pubkey_compressed.x().to_owned().unwrap())
                .unwrap();
        ...
        let pubkey_compressed = key.group_key().to_encoded_point(true);
        XOnlyPublicKey::from_slice(pubkey_compressed.x().unwrap())
            .unwrap()
            .serialize()
            .map(|x| x as i8)
            .to_vec()
...
pub struct SchnorrSignWrapper {
    pub algo_machine:
        Option<AlgorithmMachine<Secp256k1, Schnorr<Secp256k1, RecommendedTranscript, BitcoinHram>>>,
    pub sign_machine: Option<
        AlgorithmSignMachine<Secp256k1, Schnorr<Secp256k1, RecommendedTranscript, BitcoinHram>>,
    >,
    pub signature_machine: Option<
        AlgorithmSignatureMachine<
            Secp256k1,
            Schnorr<Secp256k1, RecommendedTranscript, BitcoinHram>,
        >,
    >,
    pub tweak: Scalar,
...
pub struct ScriptContainer{
    pub scripts: Vec<(u64,Vec<u8>)>
}

impl ScriptContainer{
    fn new() -> ScriptContainer{
        Self { scripts: vec![] }
    }

    fn add_script(& mut self , satoshi_amount: u64 ,script: &[i8]) {
        let script_u8 = unsafe { &*(script as *const _ as *const [u8]) };
        self.scripts.push((satoshi_amount,script_u8.into()))
    }
}
...
impl SchnorrSignWrapper {
        ...
        Self {
            algo_machine: Some(
                AlgorithmMachine::new(
                    Schnorr::<Secp256k1, RecommendedTranscript, BitcoinHram>::new(RecommendedTranscript::new(b"b")),
                    key.clone(),
                )
            ),
            sign_machine: None,
    ...
    fn sign_2_sign_bitcoin(
        wrapper: SchnorrSignWrapper,
        params: SignParams2,
        msg_i8: &[i8],
        prev_out_scripts : ScriptContainer,
    ) -> SignResult2 {
        let msg = unsafe { &*(msg_i8 as *const _ as *const [u8]) };
        //assume msg is serialized transaction
        let tx = deserialize::<Transaction>(msg).unwrap();
        // let script = Script::from_hex(hex::encode(prev_out_script).as_str()).unwrap();

        let mut tx_outs = vec![
        ];

        //todo this only works with one input. Each input needs a seperate signing procedure since each signature is for each input.
        for (amount,script) in prev_out_scripts.scripts {
            tx_outs.push(TxOut{
                value: amount,
                script_pubkey:  Script::from_hex(hex::encode(script).as_str()).unwrap()
            })
        }
        ...
        let hash = SighashCache::new(&tx)
            .taproot_key_spend_signature_hash(
                0,
                &sighash::Prevouts::All(tx_outs.as_slice()),
                SchnorrSighashType::Default,
            )
            .unwrap();
        ...
        let map: HashMap<Participant, Preprocess<_, _>> = params
            .commitments
            .iter()
            .map(|(&user, buf)| {
                let preprocess = sign_machine
                    ...
                    .unwrap();
                (Participant::new(user).unwrap(), preprocess)
            })
            .collect();
    ...
    fn sign_3_complete(wrapper: SchnorrSignWrapper, params: SignParams3) -> Vec<i8> {
        ...
        let shares_map = params
            .shares
            .iter()
            .map(|(&user, buf)| {
                let sig = signature_machine
                    .read_share::<&[u8]>(&mut buf.as_ref())
                    .unwrap();
                (Participant::new(user).unwrap(), sig)
            })
            .collect();
        ...
        let verify_key = XOnlyPublicKey::from_slice(
            wrapper
                .threshold_keys
                .group_key()
        ...
        sig.verify(&verify_msg_obj, &verify_key)
            .expect("sig is invalid");
...
impl SchnorrKeyGenWrapper {
    fn new(threshold: u16, n: u16, index: u16, context: String) -> SchnorrKeyGenWrapper {
        ...
        let key_params = ThresholdParams::new(threshold, n, Participant::new(index).unwrap()).unwrap();
        SchnorrKeyGenWrapper {
            key_params: key_params,
            key_gen_machine: Some(KeyGenMachine::<Secp256k1>::new(
                key_params.clone(),
                context.into(),
            )),
    ...
    fn key_gen_1_create_commitments(wrapper: SchnorrKeyGenWrapper) -> ResultKeygen1 {
        let (secret_share_machine, commitments) = wrapper
            .key_gen_machine
            .unwrap()
    ...
    fn key_gen_2_generate_shares(
        wrapper: SchnorrKeyGenWrapper,
        param_wrapper: ParamsKeygen2,
    ) -> ResultKeygen2 {
        let mut commitments_map = HashMap::<Participant, _>::new();
        let commitments = param_wrapper
            .commitment_user_indices
            .iter()
            .map(|i| -> Participant{
                Participant::new(*i).unwrap()
            })
            .zip(param_wrapper.commitments);
        commitments.for_each(|(i, bytes_i8)| {
            let bytes: Vec<u8> = bytes_i8.iter().map(|&x| x as u8).collect();
            let commitment =
                EncryptionKeyMessage::<Secp256k1, Commitments<Secp256k1>>::read::<&[u8]>(
                    &mut bytes.as_ref(),
                    wrapper.key_params.clone(),
                )
                .unwrap();
            commitments_map.insert(i, commitment);
        });

        let (key_machine, mut shares) = wrapper
            .secret_share_machine
            .unwrap()
            .generate_secret_shares(&mut OsRng, commitments_map)
            .unwrap();

        let mut res_wrapper = ResultKeygen2::new();

        shares.iter_mut().for_each(|(i, secret_share)| {
            let mut buffer: Vec<u8> = vec![];
            secret_share.write(&mut buffer);
            res_wrapper.user_indices.push(i.to_owned().into());
    ...
    fn key_gen_3_complete(
        wrapper: SchnorrKeyGenWrapper,
        param_wrapper: ParamsKeygen3,
    ) -> SchnorrKeyWrapper {
        let mut shares_map = HashMap::<Participant, _>::new();
        let shares = param_wrapper
            .shares_user_indices
            .iter()
            .zip(param_wrapper.shares);
        shares.for_each(|(from, bytes_i8)| {
            let bytes: Vec<u8> = bytes_i8.iter().map(|&x| x as u8).collect();
            let share =
                EncryptedMessage::<
                    Secp256k1,
                    SecretShare<<modular_frost::curve::Secp256k1 as Ciphersuite>::F>,
                >::read::<&[u8]>(&mut bytes.as_ref(), wrapper.key_params.clone())
                .unwrap();
            shares_map.insert(Participant::new(*from).unwrap(), share);
        });

        let blame_machine = wrapper
            .key_machine
            .unwrap()
            .calculate_share(&mut OsRng, shares_map)
            .unwrap();
